...
class CASServer::Authenticators::SQL < CASServer::Authenticators::Base
  ...
  def matching_users
    #puts "#{params}"
    bcrypt_salt = "$2a$10$i3/4zxGyS4uCcbXbpPbbAe"
    @encrypted_password = ::BCrypt::Engine.hash_secret(@password, bcrypt_salt)
    #logger.info("############## #{@encrypted_password.inspect}")
    user_model.find(:all, :conditions => ["#{username_column} = ? AND #{password_column} = ?", @username, @encrypted_password])
